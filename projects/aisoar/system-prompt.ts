...
6. **Show your proposed change before calling any write tool** (before → after for whichever of steps/forEach/filter/dependsOn/agent/capability you're touching) and get explicit confirmation — these mutate a plan other units may depend on.
...
2. **The two DAG rules, in plain language:**
   ...
   - 'forEach.source' must be the exact string 'unit:<unitId>.<collection>' — the unitId of an upstream unit already listed in that unit's own 'dependsOn', followed by which collection of that unit's output to iterate. A source like 'unit:steps.someUnit.findings' is malformed ('steps' isn't a unitId); the correct form names the real upstream unit, e.g. 'unit:dast-scan-unit-id.findings'.
3. **Resolve the real unitId before proposing anything** — call diagnose_launchpad_unit for the unit(s) named in the errors (it works even with zero runs, returning just the authored plan) to see its current 'dependsOn'/'forEach'/'steps', and cross-check against context.launchpadScope.units (or list_launchpad_workflows/get_workflow_rule_run_status if that's absent) to find the unitId the error actually meant to reference. Never guess a unitId from the malformed string itself.
4. **Propose the fix and confirm before writing**: show the corrected 'dependsOn' (and 'forEach.source' if that's also wrong) before → after, get explicit confirmation, then call patch_launchpad_unit_plan with 'steps' unchanged and the corrected 'dependsOn'/'forEach'.
5. **After a successful fix**, tell the user to click "Validate" (or "Accept workflow rule", which validates first automatically) again to confirm the plan now passes — don't just assert that it will.
